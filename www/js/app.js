    ...
    .config(function ($stateProvider, $urlRouterProvider) {
        $stateProvider
            ...
            .state('app.home.list', {
                ...
                views: {
                    'list': {
                        templateUrl: 'templates/home/areasList.html',
                        controller: 'AreaListsCtrl'
                    }
                }
            })
            ...
            .state('app.area', {
                ...
                views: {
                    'menuContent': {
                        templateUrl: 'templates/area/area.html',
            ...
            .state('app.area.info', {
                ...
                views: {
                    'info': {
                        templateUrl: 'templates/area/areaInfo.html',
                        controller: 'AreaListCtrl'
                    }
                }
            })
            .state('app.area.list', {
                ...
                views: {
                    'list': {
                        templateUrl: 'templates/area/topoList.html',
                        controller: 'AreaListCtrl'
                    }
                }
            })
            .state('app.area.map', {
                ...
                views: {
                    'map': {
                        templateUrl: 'templates/area/areaMap.html',
                        controller: 'AreaListCtrl'
                    }
                }
            })
...
//                    imageLazyBackgroundImage: "@imageLazyBackgroundImage",
                ...
                link: function ($scope, $element, $attributes) {
                    console.log(" imageLazySrc called");
                    var url = "img/areas/";

                    var imageName = getImageName($attributes.imageLazySrc);

                    var path =  url + $attributes.areaName +"/"+ imageName;
                    console.log("path["+path+"]");

                    function loadImage(path) {
                        var bgImg = new Image();
                        bgImg.onload = function () {
                            $element[0].style.backgroundImage = 'url(' + path + ')'; // set style attribute on element (it will load image)
                            $element[0].style.backgroundRepeat = 'no-repeat'; // set style attribute on element (it will load image)
                            var imgWidth = bgImg.width;
                            console.log("imgWidth["+imgWidth+"]");
                            var imgHeight = bgImg.height;
                            console.log("imgHeight["+imgHeight+"]");
                            var divWidth = $element[0].clientWidth;
                            console.log("divWidth["+divWidth+"]");

//                            original height / original width * new width = new height

                            var newHeight = imgHeight/imgWidth * divWidth;
                            $element[0].style.backgroundSize= divWidth+'px '+ newHeight+"px";
